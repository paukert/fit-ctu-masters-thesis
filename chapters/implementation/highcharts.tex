...
Dalším z důležitých aspektů implementace byla podpora světlého a tmavého režimu, která je realizována pomocí modulu \mintinline{text}|highcharts/themes/adaptive|. Tento modul zajišťuje automatické přizpůsobení barevného schématu grafů aktuálnímu nastavení aplikace. Detekce preferovaného režimu probíhá na základě systémové preference (pomocí CSS pravidla \mintinline{text}|prefers-color-scheme|), případně dle CSS třídy, která je dynamicky přidávána na HTML element \mintinline{text}|<body>| na~základě uživatelského nastavení.
