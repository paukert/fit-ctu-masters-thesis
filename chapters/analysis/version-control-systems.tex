...
\subsection{APIs}
GitHub i GitLab poskytují REST a GraphQL APIs, pomocí nichž lze provádět nejrůznější integrace, vytvářet nástroje na automatizaci procesů nebo mohou sloužit jako zdroj dat pro analytické dashboardy. Dokumentace těchto APIs jsou dostupné online na webových stránkách GitHubu \cite{GitHub-REST}\cite{GitHub-GraphQL} a GitLabu \cite{GitLab-REST}\cite{GitLab-GraphQL}. U veřejných repozitářů je možné u obou služeb přistupovat k některým endpointům bez autorizace, ale v ostatních případech je nutné využít některý z následujících způsobů pro získání potřebných oprávnění.

\begin{description}
    \item[GitHub] Authenticating with a personal access token
    \item[GitLab] \todo{popsat možných způsobů autorizace}
\end{description}
